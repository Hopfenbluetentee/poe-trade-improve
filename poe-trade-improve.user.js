...
// @downloadURL  https://github.com/Hopfenbluetentee/poe-trade-improve/raw/refs/heads/main/poe-trade-improve.user.js
// ==/UserScript==
...
(function () {
  ...
  (function injectStyle() {
    ...
    style.textContent = `
      #pti-bar input[type=number]::-webkit-outer-spin-button,
      #pti-bar input[type=number]::-webkit-inner-spin-button {
      ...
      #pti-bar .pti-field:focus-within { border-color: #6b5f47; }
      #pti-bar .pti-field-label {
      ...
      #pti-bar .pti-field select:focus { background: #262119; }
      #pti-bar .pti-inputs { display: inline-flex; }
      #pti-bar .pti-inputs input {
        width: 40px; padding: 0 5px; border: 0; background: #1b1813;
        color: #e0d6c0; font-size: 12px; font-family: inherit;
        text-align: center; outline: none;
      }
      #pti-bar .pti-inputs input:first-child { border-right: 1px solid #2b261d; }
      #pti-bar .pti-inputs input::placeholder { color: #554e42; }
      #pti-bar .pti-inputs input:focus { background: #262119; }
      #pti-bar .pti-clear {
      ...
      #pti-bar .pti-clear:hover { color: #fff; background: #c0392b; }
      #pti-bar .pti-cur {
      ...
      #pti-bar .pti-cur-btn img {
        display: block; width: 18px; height: 18px; object-fit: contain;
        filter: grayscale(1) brightness(1.3); opacity: .55;
        transition: filter .12s, opacity .12s;
      }
      #pti-bar .pti-cur-btn:hover { background: rgba(255,255,255,.06); }
      #pti-bar .pti-cur-btn.is-active {
        background: #2b2519; border-color: #6b5f47; color: #e8d9b5;
      }
      #pti-bar .pti-cur-btn.is-active img { filter: none; opacity: 1; }
      ...
      #pti-bar .pti-rar-btn:hover { filter: brightness(1.3); transform: translateY(-1px); }
      #pti-bar .pti-rar-btn:active { transform: translateY(0); }
      ...
      #pti-bar .pti-rar-btn.is-struck::after {
      ...
      #pti-bar .pti-rar-clear:hover {
        background: #c0392b; border-color: #e0705f; color: #fff; filter: none;
      }
      #pti-bar .power-control-btn { border-radius: ${RADIUS}; }
      #pti-bar .power-control-btn:hover { background: rgba(255,255,255,.06); }
      #pti-bar .power-control-mirror:hover { filter: brightness(1.15); }
  ...
  function waitFor(test, onFound, timeout = 20000, label = 'unlabeled') {
    ...
    if (immediate) {
      if (DEBUG) console.log(`[PTI/wait:${label}] found immediately`);
      return onFound(immediate);
    }
    if (DEBUG) console.log(`[PTI/wait:${label}] not found yet, observing...`);
    ...
    observer = new MutationObserver(() => {
      ...
      if (!found) return;
      stop();
      if (DEBUG) console.log(`[PTI/wait:${label}] found via mutation`);
      onFound(found);
    });
    ...
    timer = setTimeout(() => {
      stop();
      console.warn(`[PTI] timed out waiting for element: ${label}`);
    }, timeout);
  ...
  function findControlsBar() {
    ...
    if (direct && direct.id !== 'pti-bar') {
      if (DEBUG) console.log('[PTI/tab] findControlsBar: matched direct selector', direct);
      return direct;
    }
    ...
    if (liveButton) {
      ...
      if (container && container.id !== 'pti-bar') {
        if (DEBUG) console.log('[PTI/tab] findControlsBar: matched via live button', container);
        return container;
      }
    }
    if (DEBUG) console.log('[PTI/tab] findControlsBar: nothing found this pass');
    return null;
  }
  ...
  function enableGroupFor(element) {
    ...
    const toggle = groupToggleOf(group);
    if (DEBUG) console.log('[PTI/group] activating:', toggle.className);
    ...
    if (!enabled) console.warn('[PTI/group] toggle did not switch:', toggle.className);
    return enabled;
  }
  ...
  function applyFilterLock(locked) {
    ...
    if (DEBUG) console.log('[PTI/lock] filters locked:', locked);
      ...
      const update = () => {
        if (DEBUG) {
          ...
          if (signature !== lastSignature) {
            ...
            console.log('[PTI/lock] live button signature:', signature);
          }
        }
  ...
  function selectOption(multiselect, optionText) {
    ...
    const log = (...args) => {
      if (SLOW_SELECT || DEBUG) console.log('[PTI/select]', ...args);
    };
    ...
    setTimeout(() => {
      ...
      if (!optionEl) {
        console.warn('[PTI/select] exact option not in list:',
  ...
  function findOptionText(multiselect, wanted) {
    ...
    if (!match) {
      console.warn('[PTI] option not found:', wanted, '- available:', texts);
    }
    return match || null;
  }
...
${banned ? `<span style="position:absolute;inset:0;">${banSvg(size)}</span>` : ''}
  ...
  function addTristateButton(slot, config) {
    ...
    waitForMultiselectFilter(filterTitle, (filter) => {
      ...
      if (DEBUG) console.log('[PTI] tristate filter ready:', filterTitle);
  ...
  function addRarityMirror(slot, config) {
    ...
    const box = document.createElement('span');
    box.className = 'pti-cur';
    slot.appendChild(box);

    waitForMultiselectFilter(filterTitle, (filter) => {
      const multiselect = filter.querySelector('.multiselect');
      if (DEBUG) console.log('[PTI] rarity filter ready:', filterTitle);
      ...
      const buttons = RARITIES.map((rarity) => {
        const button = document.createElement('button');
        button.type = 'button';
        button.className = rarity.struck ? 'pti-rar-btn is-struck' : 'pti-rar-btn';
      ...
      separator.className = 'pti-rar-sep';
      ...
      clearButton.className = 'pti-rar-btn pti-rar-clear';
  ...
  function addMirroredButton(slot, spec, fallbackCaption, options) {
    ...
    waitFor(() => findSiteButton(spec), (original) => {
      ...
      if (DEBUG) console.log('[PTI] mirrored button:', fallbackCaption,
  ...
  function addFloatingActionBar() {
    waitFor(() => document.querySelector('.search-advanced-items'), (container) => {
      if (container.querySelector('.pti-floating-actions')) return;

      const wrap = document.createElement('div');
      wrap.className = 'pti-floating-actions';
  ...
  function clipControls(element, clipped) {
    ...
    if (clipped) {
      ...
      if (DEBUG) console.log('[PTI] clipped:', element.className);
    } else {
  ...
  function mountBar(controls) {
    if (document.getElementById('pti-bar')) return null;
    ...
    if (advanced && advanced.parentElement) {
      ...
      if (DEBUG) console.log('[PTI] bar inserted before search-advanced');
    } else {
  ...
  function isMirrorUIStale() {
    if (!activeUI) {
      if (DEBUG) console.log('[PTI/tab] stale check: no activeUI yet');
      return true;
    }
    if (!activeUI.controls.isConnected) {
      if (DEBUG) console.log('[PTI/tab] stale check: controls container detached');
      return true;
    }
    if (!activeUI.bar.isConnected) {
      if (DEBUG) console.log('[PTI/tab] stale check: our own bar detached');
      return true;
    }
    ...
    for (const original of trackedOriginals) {
      if (!original.isConnected) {
        if (DEBUG) console.log('[PTI/tab] stale check: a tracked filter input is detached', original);
        return true;
      }
    }
    if (DEBUG) {
      console.log('[PTI/tab] stale check: everything still connected',
  ...
  function teardownMirrorUI() {
    if (DEBUG) console.log('[PTI/tab] tearing down stale mirror UI');
    // Stop every watcher first, so nothing keeps polling the nodes we
    // are about to drop.
    disposeAll();
    const oldBar = document.getElementById('pti-bar');
    if (oldBar) oldBar.remove();
    const oldFloating = document.querySelector('.pti-floating-actions');
  ...
  function mountMirrorUI(controls) {
    if (DEBUG) console.log('[PTI/tab] mounting mirror UI against controls:', controls);
    const slots = mountBar(controls);
    if (!slots) {
      console.warn('[PTI] bar already present, skipping mount - ' +
    ...
    if (DEBUG) console.log('[PTI/tab] mount complete, activeUI set:', activeUI);
  }
  ...
  function ensureMirrorUI(onReady) {
    if (!isMirrorUIStale()) {
      if (DEBUG) console.log('[PTI/tab] ensureMirrorUI: not stale, skipping rebuild');
      return onReady();
    }

    if (DEBUG) console.log('[PTI/tab] ensureMirrorUI: stale, rebuilding');
    teardownMirrorUI();
  ...
  const applyTabState = () => {
    ...
    if (DEBUG) console.log('[PTI/tab] applyTabState fired: active =', active, '| wasActive =', wasActive);
    ...
    if (!active) {
      ...
      if (DEBUG) console.log('[PTI] search tab active:', active);
      return;
    }

    ensureMirrorUI(() => {
      if (!activeUI) {
        console.warn('[PTI/tab] ensureMirrorUI onReady but activeUI is still null');
        return;
      }
  ...
  ensureMirrorUI(() => {
    ...
    if (DEBUG) console.log('[PTI/tab] tab list element:', tabList);
    if (tabList) {
      new MutationObserver(applyTabState).observe(tabList, {
        attributes: true, attributeFilter: ['class'], subtree: true
      });
    } else {
      console.warn('[PTI] tab list not found, polling instead');
